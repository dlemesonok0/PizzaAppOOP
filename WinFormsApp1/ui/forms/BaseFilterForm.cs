...
{
    private ISpecification<PizzaBase> _spec;
    ...
    {
        List<ISpecification<PizzaBase>> specs = new();

        if (nameFilterCheckBox.Checked && !string.IsNullOrWhiteSpace(nameTextBox.Text))
        {
            specs.Add(new NameContainsSpecification<PizzaBase>(nameTextBox.Text));
        }

        if (costLessFilterCheckBox.Checked && costLessTextBox.Value > 0)
        {
            specs.Add(new CostLessThanSpecification<PizzaBase>(costLessTextBox.Value));
        }

        if (costMoreFilterCheckBox.Checked && costMoreTextBox.Value > 0)
        {
            specs.Add(new CostMoreThanSpecification<PizzaBase>(costMoreTextBox.Value));
        }
        ...
        {
            _spec = specs.Aggregate((current, next) => new AndSpecification<PizzaBase>(current, next));
        }
    ...
    public ISpecification<PizzaBase> GetSpecification() => _spec;
}